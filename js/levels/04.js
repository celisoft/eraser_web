...
function lvl04_create()
{
	...
	//Generating grounds
	var ground = platforms.create(1*64, 3*64, 'ground')
	ground.scale.setTo(2, 1);
	ground.body.immovable = true;
  	ground.body.checkCollision.down = false;

	ground = platforms.create(12*64, 4*64, 'ground');
	ground.scale.setTo(5, 1);
	ground.body.immovable = true;
  	ground.body.checkCollision.down = false;

	ground = platforms.create(1*64, 5*64, 'ground');
	ground.scale.setTo(2, 1);
	ground.body.immovable = true;
  	ground.body.checkCollision.down = false;

	ground = platforms.create(1*64, 7*64, 'ground');
	ground.scale.setTo(2, 1);
	ground.body.immovable = true;
  	ground.body.checkCollision.down = false;
	...
	//Integrate exit
	exit_door = this.game.add.sprite(1*64, 1*64, 'hole');
	...
	var arachne = game.add.sprite(2*64, 7*64+16, 'arachne');
	arachne.animations.add('moving', [0, 1, 2, 1], 2, true);
	arachne.animations.play('moving');
	dangers.add(arachne);

	var plant = game.add.sprite(1*64, 6*64, 'plant');
	...
	var ghost = ghosts.create(9*64,7*64, 'ghost');
	ghost.body.gravity.y = 250;

	// Timers
	timers = game.add.group();
	timers.enableBody = true;
	var timer = timers.create(2*64,6*64, 'timer');
	...
	//Integrate player
	player = game.add.sprite(12*64, 0, 'playersheet');
